...
export async function generateMetadata({ params }: Props) {
  ...
  const sharedMetadata = {
		title,
		description,
		images: typeof seo.image === 'object' ? contructImageUrl(seo.image._key as string) : "https://570pc5yjce.ufs.sh/f/QUFIlUYDwcG5n17hl4jqNAQCs5rbeKtmhHl4F1Uwdg9zSLkx",
	};
...
const page = async ({ params }: Props) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <BlogRenderer blogData={blogData} draft={draft} blogViews={blogViews} blogId={page.docs.id} createdAt={page.docs.createdAt} />
    </div>
  )
}
