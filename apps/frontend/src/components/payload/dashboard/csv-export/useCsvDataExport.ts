...
export const fetchPagesData = async (pagesCount: number): Promise<Page[]> => {
    const pagesResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/pages?limit=${pagesCount}`)
...
export const useCsvDataExport = () => {
    ...
    useEffect(() => {
        const clickOutside = (e: MouseEvent) => {
            if (e.target instanceof HTMLElement && !e.target.closest('.csv-modal') && !e.target.closest('.csv-modal-trigger')) {
